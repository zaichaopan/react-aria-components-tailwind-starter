    ...
    <Button
      ...
      className={composeRenderProps(
        ...
        (className, { isFocusVisible, isPressed, isHovered, isDisabled }) =>
          twMerge(
            'relative flex w-full cursor-default items-center gap-x-1 rounded-md border text-start outline-hidden transition',
            'pe-8 ps-3',
            ...
            isFocusVisible &&
              'border-ring ring-2 ring-ring/30 group-data-[invalid]:border-ring',
            selectBoxIndicator,
            className,
          ),
      )}
...
export function SelectSection<T extends object>(props: SectionProps<T>) {
  return (
    <RACListBoxSection
      className={twMerge(
        'not-first:mt-1.5',
        'not-first:border-t not-first:border-t-border/40',
        props.className,
      )}
    >
      <Header
        className={twMerge(
          ...
          'inset-0 rounded-sm',
          'in-data-[selected-icon-placement=end]:px-2',
        )}
      >
        {props.title}
      </Header>
